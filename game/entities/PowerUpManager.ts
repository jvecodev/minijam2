...
// Interface para os power-ups
export interface PowerUp extends Position, Dimensions {
  id: number;
  angle: number;
  type: PowerUpType;
  active: boolean;
...
/**
 * Gerenciador de power-ups usando Object Pool
 */
export class PowerUpManager {
  ...
  constructor(p5: p5Types, centerX: number, ringWidth: number) {
    ...
    this.powerUpPool = new ObjectPool<PowerUp>(
      // Factory: cria novos power-ups
      () => this.createPowerUp("shield", 0),
      // Reset: reinicializa power-ups para reutilização
   ...
   */
  private createPowerUp(type: PowerUpType, angle: number): PowerUp {
    // Posição inicial
    let x = this.centerX;
    let y = this.centerY;
    ...
    return {
      ...
      particleTimer: 0,
      glowSize: 1.0 + Math.random() * 0.5,
      glowOpacity: 0.6 + Math.random() * 0.4,
      trailPoints: []
    };
  }

  /**
   * Reinicializa um power-up para ser reutilizado
   */
  ...
  update(gameSpeed: number, deltaTime: number): void {
    ...
    // Remove power-ups inativos da lista e os devolve ao pool
    const activePowerUps = this.powerUps.filter(pu => pu.active);
    
    // Devolve power-ups inativos ao pool
  ...
  draw(): void {
    ...
    for (const powerUp of this.powerUps) {
      ...
      this.drawPowerUpGlow(powerUp, time);
      
      // Desenha o power-up baseado no tipo
      this.p5.push();
      ...
      switch (powerUp.type) {
        case "shield":
          this.drawShield(powerUp, time);
          break;
        case "speed":
          this.drawSpeed(powerUp, time);
          break;
        case "life":
          this.drawLife(powerUp, time);
          break;
      }
  ...
  /**
   * Desenha o brilho externo do power-up
   */
  private drawPowerUpGlow(powerUp: PowerUp, time: number): void {
  ...
  /**
   * Desenha o power-up de escudo
   */
  private drawShield(powerUp: PowerUp, time: number): void {
    this.p5.push();
  ...
  /**
   * Desenha o power-up de velocidade
   */
  private drawSpeed(powerUp: PowerUp, time: number): void {
    this.p5.push();
  ...
  /**
   * Desenha o power-up de vida extra
   */
  private drawLife(powerUp: PowerUp, time: number): void {
    this.p5.push();
  ...
  /**
   * Gera um novo power-up
   */
  spawnPowerUp(angle: number = 0): PowerUp {
    ...
    // Reinicia as propriedades do power-up
    powerUp.type = type;
    powerUp.angle = angle;
    powerUp.active = true;
    powerUp.animation = 0;
    powerUp.pulseFrequency = 1 + Math.random() * 2;
    powerUp.rotationSpeed = (Math.random() - 0.5) * 2;
    powerUp.orbitRadius = 4 + Math.random() * 6;
    powerUp.orbitPhase = Math.random() * Math.PI * 2;
    powerUp.particleTimer = 0;
    powerUp.glowSize = 1.0 + Math.random() * 0.5;
    powerUp.glowOpacity = 0.6 + Math.random() * 0.4;
    powerUp.trailPoints = [];
    ...
    // Adiciona à lista de power-ups ativos
  ...
  /**
   * Desativa um power-up e o devolve ao pool
   */
   ...
   */
  clearPowerUps(): void {
    // Devolve todos os power-ups ao pool
    for (const powerUp of this.powerUps) {
      this.powerUpPool.release(powerUp);
    }
